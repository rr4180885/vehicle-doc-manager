...
const sizes = {
  sm: { box: "h-10 w-10", icon: "h-5 w-5", title: "text-sm", sub: "text-[10px]" },
    ...
    <div className={cn("flex items-center gap-3 min-w-0", className)}>
      <div
        className={cn(
          ...
          logoUrl ? "bg-white" : "bg-primary text-primary-foreground"
        )}
      >
        {logoUrl ? (
          <img
      ...
      {showText && (
        <div className="min-w-0 flex-1">
          ...
          <p
            className={cn(
              s.sub,
              "font-medium tracking-wide uppercase truncate mt-0.5",
