...
export class DatabaseStorage implements IStorage {
  async getVehicles(userId: string, search?: string): Promise<VehicleWithDocuments[]> {
    let query = db.select().from(vehicles).where(eq(vehicles.userId, userId));
    ...
    for (const vehicle of vehiclesList) {
      const docs = await db.select().from(documents).where(eq(documents.vehicleId, vehicle.id));
  ...
  async getVehicle(id: number): Promise<VehicleWithDocuments | undefined> {
    ...
    const docs = await db.select().from(documents).where(eq(documents.vehicleId, id));
    return { ...vehicle[0], documents: docs };
  }

  async createVehicle(userId: string, vehicle: InsertVehicle): Promise<Vehicle> {
    const [newVehicle] = await db.insert(vehicles).values({ ...vehicle, userId }).returning();
    return newVehicle;
  }
  ...
  async createVehicleWithDocuments(userId: string, data: CreateVehicleWithDocuments): Promise<VehicleWithDocuments> {
    ...
    const [newVehicle] = await db.insert(vehicles).values({ ...vehicleData, userId }).returning();
